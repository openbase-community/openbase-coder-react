...
const Settings: React.FC = () => {
  ...
  const isNativeShell = getRuntimeShell() === "electron";
    ...
    const activeSection = SETTINGS_SECTIONS.find((section) => section.id === settingsSection)!;
              ...
              <nav aria-label="Settings categories" className="space-y-1">
                {SETTINGS_SECTIONS.map((section) => (
                  <button
                    ...
                    className={`group flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition-colors ${settingsSection === section.id ? "bg-white text-primary shadow-[0_1px_2px_rgba(24,73,139,.08),inset_0_0_0_1px_rgba(24,73,139,.05)]" : "text-muted-foreground hover:bg-white/55 hover:text-foreground"}`}
                    key={section.id}
                    onClick={() => setSettingsSection(section.id)}
                    type="button"
                  >
                    <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] ${settingsSection === section.id ? "bg-primary/[0.09] text-primary" : "bg-white/55 text-muted-foreground"}`}>
                    ...
                    <ChevronRight className={`h-4 w-4 transition-transform ${settingsSection === section.id ? "translate-x-0 text-primary" : "-translate-x-1 opacity-0 group-hover:translate-x-0 group-hover:opacity-50"}`} />
                  </button>
                ))}
              </nav>
            </aside>

            <section className="min-w-0 p-6 lg:p-8" key={settingsSection}>
              ...
              <div className="ob-settings-content space-y-4">{sectionContent[settingsSection]}</div>
            </section>
          </div>
        </div>
      </DashboardLayout>
