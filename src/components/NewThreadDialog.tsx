...
import { ArrowRight, Folder, FolderOpen, LoaderCircle, Search } from "lucide-react";
import {
  useEffect,
  useMemo,
  useState,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setOpen}>
      ...
      <DialogContent
        className={
          isDesktop
            ? "ob-new-thread-dialog gap-0 overflow-hidden p-0 sm:max-w-[620px]"
            : "sm:max-w-md"
        }
      >
        {!isDesktop ? (
          <>
            ...
            <div className="mt-1 max-h-72 space-y-px overflow-y-auto">
              {loading ? (
                ...
                <p className="py-6 text-center text-[12px] text-destructive">{error}</p>
              ) : projects.length === 0 ? (
                <p className="py-6 text-center text-[12px] text-muted-foreground">
                  No recent projects
                </p>
