...
Shared React UI package for Openbase.

This package contains the reusable application shell, pages, hooks, shared UI components, plugin registry helpers, and CSS used by the Openbase browser console and desktop app. It is consumed from the workspace as `@openbase/coder-react`.

## Usage

The package is intended to be used from the Openbase workspace:
...
- `@openbase/coder-react/App`: the shared Openbase React app and routes.
...
- `console`: browser-hosted Openbase frontend.
- `desktop`: Electron desktop shell for Openbase.
